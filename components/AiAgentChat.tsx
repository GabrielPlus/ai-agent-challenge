...
function AiAgentChat({ videoId }: { videoId: string }) {
  ...
  useEffect(() => {
    ...
    switch (status) {
      case "submitted":
        toastId = toast("CortexAI is thinking...", {
          id: toastId,
          icon: <BotIcon className="w-4 h-4" />,
        });
        break;
      case "streaming":
        toastId = toast("CortexAI is replying...", {
          id: toastId,
          icon: <BotIcon className="w-4 h-4" />,
        });
        break;
        case "error":
          ...
          console.error("Chat Error:", status, messages);
          break;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="hidden lg:block px-4 pb-3 border-b border-gray-100">
        <h2 className="text-lg font-semibold text-gray-800">CortexAI Agent</h2>
      </div>
            ...
            <div className="flex items-center justify-center h-full min-h-[200px]">
              <div className="text-center space-y-2">
                <h3 className="text-lg font-medium text-gray-700">
                  Welcome to CortexAI Agent Chat
                </h3>
              ...
              >
                {m.parts && m.role === "assistant" ? (
                  ...
                  <div className="space-y-3">
                    {m.parts.map((part, i) =>
                      part.type === "text" ? (
                        <div key={i} className="markdown-content max-w-none">
                  ...
                  // User message
                  <div className="markdown-content max-w-none text-white">
  ...
  className="px-4 py-2 bg-blue-500 text-white text-sm rounded-full hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
>
  {status === "streaming" ? (
    <>
      <svg
